...
package body Crc16 is
  ...
  16#8201#, 16#42C0#, 16#4380#, 16#8341#, 16#4100#, 16#81C1#, 16#8081#, 16#4040#
   ...
   procedure Update( Oldvalue : Interfaces.Unsigned_16 ;
		     ...
		     Blocklen : Integer ;
		     Newvalue : out Interfaces.Unsigned_16 ) is
      use Interfaces ;
      Crclo : Unsigned_8 ;
      Newcrc : Unsigned_16 := Oldvalue ;
      Block : array(1..Blocklen) of Unsigned_8 ;
      for Block'Address use Blockptr ;
   begin
      for Idx in 1..Blocklen
      loop
	 Crclo := Unsigned_8(newcrc and 16#00ff# ) ;
	 Crclo := Crclo xor Block(Idx) ;
	 Newcrc := Shift_Right(Newcrc,8) ;
	 Newcrc := Newcrc xor CRCtable(1+Integer(crclo)) ;
      end loop ;      
      Newvalue := Newcrc ;
   end Update ;
   ...
   procedure Update( crcvalue : in out interfaces.unsigned_16 ;
                     block : system.Storage_Elements.Storage_Array ) is
   ...
   procedure Selftest is
      Crcoftable : Interfaces.Unsigned_16 := 0 ;
      alpha : string := "abcdefghijklmnopqrstuvwxyz" ;
   begin
      Update( Crcoftable , Crctable'Address , 2 * Crctable'Length , Crcoftable ) ;
      Put("CRC of the table is ");
      Put_Line(interfaces.Unsigned_16'Image(Crcoftable)) ;
      Crcoftable := 0 ;
      Update( Crcoftable , alpha'address , alpha'length , crcoftable ) ;
      put("CRC of "); put(alpha) ; put(" is "); put(interfaces.unsigned_16'image(crcoftable)) ;
      new_line ;
   end Selftest ;
